...
async fn parse_short_array(stream: &mut Pin<Box<impl Stream<Item=u8>>>) -> Result<Array, AppError> {
    ...
    let element_constructor = stream.next().await.ok_or(AppError::IteratorEmptyOrTooShortError)?;
    Ok(Array(parse_raw_to_vec(stream, size as usize, count as usize, element_constructor).await?))
}
...
async fn parse_raw_to_vec(stream: &mut Pin<Box<impl Stream<Item=u8>>>, size: usize, count: usize, element_constructor: u8) -> Result<Vec<AmqpType>, AppError> {
    let mut result = Vec::with_capacity(count);
    let mut buffer = Box::pin(iter(read_bytes(stream, size).await?));
    for _ in 0..count {
        let amqp_type = Box::pin(AmqpType::try_decode_with_constructor(element_constructor, &mut buffer)).await?;
...
mod test {
    ...
    async fn try_decode_short_array_returns_correct_value() {
        ...
        let res = Array::try_decode(ARRAY_SHORT, &mut bytes.into_pinned_stream()).await.unwrap();
    ...
    async fn try_decode_array_returns_correct_value() {
        let bytes = vec![0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x01, INTEGER, 0x00, 0x00, 0x00, 0x15];
        let res = Array::try_decode(ARRAY, &mut bytes.into_pinned_stream()).await.unwrap();
        assert_eq!(res.0.len(), 1);
        match res.0[0] {
            AmqpType::Int(value) => assert_eq!(value, 21),
            _ => panic!("Could not destructure expected int value"),
    ...
    async fn try_decode_short_array_returns_error_if_constructor_is_wrong() {
        ...
        assert!(matches!(res, Err(AppError::DeserializationIllegalConstructorError(_))));
    }

    #[tokio::test]
    async fn try_decode_short_array_returns_error_if_element_constructor_is_wrong() {
        let bytes = vec![0x04, 0x01, 0x99 /*<---wrong element constructor*/, 0x00, 0x00, 0x00, 0x15];
    ...
    async fn try_decode_array_returns_error_if_element_constructor_is_wrong() {
        let bytes = vec![0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x01, 0x99 /*<---wrong element constructor*/, 0x00, 0x00, 0x00, 0x15];
