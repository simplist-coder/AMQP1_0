...
async fn parse_short_map(stream: &mut Pin<Box<impl Stream<Item=u8>>>) -> Result<Map, AppError> {
    ...
    Ok(Map(parse_to_index_map(stream, size as usize, count as usize).await?))
}
...
async fn parse_to_index_map(stream: &mut Pin<Box<impl Stream<Item=u8>>>, size: usize, count: usize) -> Result<IndexMap<AmqpType, AmqpType>, AppError> {
    if count % 2 != 0 {
...
mod test {
    ...
    async fn try_decode_short_map_returns_correct_value() {
        ...
        let res = Map::try_decode(MAP_SHORT, &mut bytes.into_pinned_stream()).await.unwrap();
    ...
    async fn try_decode_map_returns_correct_value() {
        let bytes = vec![0x00, 0x00, 0x00, 8, 0x00, 0x00, 0x00, 2, INTEGER, 0x00, 0x00, 0x00, 21, UNSIGNED_SHORT, 0x00, 16];
        let res = Map::try_decode(MAP, &mut bytes.into_pinned_stream()).await.unwrap();
        assert_eq!(res.0.len(), 1);
        assert!(matches!(&res.0[&AmqpType::Int(21)], AmqpType::Ushort(16)));
    }
    ...
    async fn try_decode_short_map_returns_error_if_constructor_is_wrong() {
        ...
        let res = Map::try_decode(ILLEGAL_ELEMENT_CONSTRUCTOR, &mut bytes.into_pinned_stream()).await;
        assert!(matches!(res, Err(AppError::DeserializationIllegalConstructorError(ILLEGAL_ELEMENT_CONSTRUCTOR))));
    }

    #[tokio::test]
    async fn try_decode_short_map_returns_error_if_element_constructor_is_wrong() {
        let bytes = vec![8, 2, ILLEGAL_ELEMENT_CONSTRUCTOR , 0x00, 0x00, 0x00, 21, UNSIGNED_SHORT, 0x00, 16];
    ...
    async fn try_decode_map_returns_error_if_element_constructor_is_wrong() {
        let bytes = vec![0x00, 0x00, 0x00, 8, 0x00, 0x00, 0x00, 2, ILLEGAL_ELEMENT_CONSTRUCTOR, 0x00, 0x00, 0x00, 21, UNSIGNED_SHORT, 0x00, 16];
    ...
    async fn try_decode_short_map_returns_error_number_of_elements_is_odd() {
        let bytes = vec![5, 1, INTEGER, 0x00, 0x00, 0x00, 21];
        let res = Map::try_decode(MAP_SHORT, &mut bytes.into_pinned_stream()).await;
        assert!(matches!(res, Err(AppError::DeserializationMapContainsOddAmountOfElementsError)));
    }
